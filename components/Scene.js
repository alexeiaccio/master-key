  ...
  return (
    <div className="fixed w-screen h-screen">
      {!started ? (
        <>
          <div
            className="absolute inset-0 flex-col items-center justify-center hidden text-xl text-black cursor-pointer md:flex"
            onClick={handleStart}
          >
            <div>{lang === 'eng' ? 'click to start' : 'клик чтобы начать'}</div>
            <div>{lang === 'eng' ? '(fullscreen)' : '(полный экран)'}</div>
          </div>
          <div className="absolute inset-0 flex flex-col items-center justify-center text-xl text-black md:hidden">
            <div>
              {lang === 'eng'
                ? 'open on desktop'
